...
procedure {:sourceloc "MainContract.bidIncreasesAssets.s.sol", 14, 5} {:message "MainContract::bid"} bid#35(__this: address_t, __msg_sender: address_t, __msg_value: int)
	...
	requires {:sourceloc "MainContract.bidIncreasesAssets.s.sol", 14, 5} {:message "Precondition 'msg.value > currentBid' might not hold when entering function."} (__msg_value > currentBid#5[__this]);
	...
	ensures {:sourceloc "MainContract.bidIncreasesAssets.s.sol", 14, 5} {:message "Postcondition 'address(this).balance > __verifier_old_uint(address(this).balance)' might not hold at end of function."} (__balance[__this] > old(__balance[__this]));

{
	...
	assume (__msg_value >= currentBid#5[__this]);
	assume {:sourceloc "MainContract.bidIncreasesAssets.s.sol", 17, 9} {:message ""} true;
	call __transfer(currentBidder#3[__this], __this, 0, currentBid#5[__this]);
...
// 
// Default constructor
procedure {:sourceloc "MainContract.bidIncreasesAssets.s.sol", 4, 1} {:message "MainContract::[implicit_constructor]"} __constructor#36(__this: address_t, __msg_sender: address_t, __msg_value: int)
{
