...
function callnotchecked_unchk25(address payable callee) public {
    ...
    function actualBalanceOf(address _owner) public view virtual returns (uint256 balance);
...
function my_func_unchk23(address payable dst) public payable{
    ...
    {
        proposals.push(); proposalID = proposals.length - 1;
...
function sendToWinner_unchk8() public {
        ...
        returns (uint voteID)
    {
        ...
        p.votes.push(); voteID = p.votes.length - 1;
...
function bug_unchk39(address payable addr) public
    ...
    function executeProposal(uint proposalNumber, bytes memory transactionBytecode) public {
        ...
        require(initialized);
        require(block.timestamp > p.minExecutionDate                                             // If it is past the voting deadline
