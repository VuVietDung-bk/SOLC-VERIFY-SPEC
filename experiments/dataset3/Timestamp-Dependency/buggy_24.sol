...
function play_tmstmp6(uint startTime) public {
  ...
  function bug_tmstmp16 () public payable {
	...
	require(msg.value == 10 ether); // must send 10 ether to play
        require(block.timestamp != pastBlockTime_tmstmp16); // only 1 transaction per block   //bug
        pastBlockTime_tmstmp16 = block.timestamp;       //bug
  ...
  function bug_tmstmp24 () public payable {
	...
	require(msg.value == 10 ether); // must send 10 ether to play
        require(block.timestamp != pastBlockTime_tmstmp24); // only 1 transaction per block   //bug
        pastBlockTime_tmstmp24 = block.timestamp;       //bug
        if(block.timestamp % 15 == 0) { // winner    //bug
            payable(msg.sender).transfer(address(this).balance);
        }
    }
...
function play_tmstmp15(uint startTime) public {
  ...
  function bug_tmstmp28 () public payable {
	...
	require(msg.value == 10 ether); // must send 10 ether to play
        require(block.timestamp != pastBlockTime_tmstmp28); // only 1 transaction per block   //bug
        pastBlockTime_tmstmp28 = block.timestamp;       //bug
        if(block.timestamp % 15 == 0) { // winner    //bug
            payable(msg.sender).transfer(address(this).balance);
        }
    }
    ...
    function invest(address sponsorAddress, uint256 investCycle) external payable {
        ...
        if (user.totalInvestCount > 0) {
            ...
            userMapping[msg.sender] = User(1, investAmount, 0, 0, 0, 1, 0, investAmount,
                                           block.timestamp, investAmount, investCycle, level,
...
function play_tmstmp23(uint startTime) public {
    ...
    function userWithdraw() external {
        ...
        if (user.currentInvestAmount > 0) {
            ...
            payable(msg.sender).transfer(avaliableIA);
        }
    }
...
function play_tmstmp14(uint startTime) public {
    ...
    function userWithdrawCommission() external {
        ...
        if (avaliableWithdrawAmount >= ONE_ETH.div(10)) {
            ...
            if (avaliableSCB > 0) {
                addStaticCommissionRecord(msg.sender, block.timestamp, avaliableSCB);
            }
            payable(msg.sender).transfer(avaliableWithdrawAmount);
        }
    }
...
function play_tmstmp30(uint startTime) public {
    ...
    function engineerWithdraw() external onlyEngineer {
        ...
        if (avaliableAmount > 0) {
            ...
            payable(msg.sender).transfer(avaliableAmount);
        }
    }
