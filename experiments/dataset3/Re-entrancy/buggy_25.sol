...
function withdrawFunds_re_ent38 (uint256 _weiToWithdraw) public {
  ...
  function placeBet(uint64 _fixtureId, uint16 _selectedTeam, uint32 _odd) external payable  {
    ...
    require( block.timestamp < ( gameList[_fixtureId].timestamp  - 10 minutes ) );

    // Save the betting information
    betList[_fixtureId].push(BetFixture( payable(msg.sender), stake,  _odd, _selectedTeam));
...
address payable lastPlayer_re_ent30;
  ...
  // Receive ether and forward to owner
