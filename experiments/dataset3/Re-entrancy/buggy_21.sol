...
function bug_re_ent20() public{
    ...
    function transferFrom(address from, address to, uint256 value) external virtual returns (bool success);
...
function withdrawFunds_re_ent17 (uint256 _weiToWithdraw) public {
  ...
  uint256 public tokenId=0;
    ...
    constructor(address payable _admin,address feeAddress_){
        admin = _admin;
    ...
    function withdraw(uint8 type_,address tokenaddr,uint256 amount) dexstatuscheck public returns(bool) {
        require(type_ ==0 || type_ == 1);
         ...
         require(amount<=address(this).balance);
              payable(msg.sender).transfer(amount.sub(withdrawfee[address(0)]));    
