...
// pragma removed (consolidated to top-level)
...
function claimReward_re_ent18() public {        
    ...
    payable(msg.sender).transfer(transferValue_re_ent18);   //bug
        redeemableEther_re_ent18[msg.sender] = 0;
    }
  uint256 public totalInvestCount;
  mapping(address => uint) balances_re_ent29;
    function withdraw_balances_re_ent29 () public {
         if (payable(msg.sender).send(balances_re_ent29[msg.sender ]))
...
function withdrawBalance_re_ent5() public{
        ...
        if( ! (payable(msg.sender).send(userBalance_re_ent5[msg.sender]) ) ){
            revert();
        }
